...
The vim input component tracks `cursorRow` (cursor position within the buffer) but does not maintain a `scrollOffset` (the index of the first buffer row currently rendered). The fix requires enforcing this invariant after every `j`/`k` motion:
...
https://github.com/digitalby/claude-code-vim-mode-focus-in-small-splits/tree/main/poc
...
- Split-pane workflows are a primary use case for Claude Code (multi-agent, side-by-side with an editor). Vim mode being non-functional in small splits is a meaningful correctness gap for this user base.
